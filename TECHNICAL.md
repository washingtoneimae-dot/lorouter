...
- Also verified inside an already-provisioned 5-dimension space (not just the original 4-dimension case): swapping one expert produced 0.00% change in the other four domains' error, and 0/4 top-1 routing decisions changed. Reproduce: `scripts/addition_isolation_suite.py`, `section_2b_swap_isolation_5dim()`.
...
Adding one new domain (law) to a 4-domain pool, then recalibrating jointly (the broken baseline). Generated fresh by this package's own code — reproduce: `scripts/addition_isolation_suite.py`, `section_3_addition_and_gating()`:
...
Two of four untouched domains show real collateral damage (creative, math); the other two happen to show none in this run — consistent with §6.5's finding that the failure concentrates on specific poorly-resolved regions rather than degrading every domain uniformly.
...
- 94.4% of requests keep $\tau$ within 2× of baseline (essentially undisturbed).
...
| `moat_calibration_trial.py` | §8 replication on the moat corpus (current: v3 brick 3, 3,010 examples): (1) addition-flip test — 7 flips at v3 size (small, real; the §6.3 gated fix is its remedy); (2) gate calibration clean-only vs clean+boundary — at p99 the §8 property replicates (threshold 0.0448 → 0.7638, false-capture 1.79% → 0.00%, recall 97.3% → 93.3%); at p95 the effect is real but weaker (false-capture 4.02% → 1.34%) — the six-domain space is harder than the four-domain one. |
| `build_moat_corpus.py` | Seeded, deterministic generator for the earlier moat corpus bricks (bricks 1–2 and the pre-v3 brick 3: up to 664 examples, 4 domains). The current `corpus/moat_brick3.jsonl` is the user-extended v3 — 3,010 examples, six domains, 220 boundaries, version tag `v3-moat-brick-3-6domain-psychology` — schema-compatible with the generator's output; the v3 generating tool is not in this repo. |
...
## 14. Performance Data Workbook

`performance_data.xlsx` is generated by `build_workbook.py` from this document's canonical run. It is not a separate data source — every number in it either matches a figure already stated above, or is a fresh aggregate computed from the same run for a view this document doesn't present in table form. Regenerate it any time with `python3 scripts/build_workbook.py` (writes to the repository root, one level up from the script's own `scripts/` location, so it runs correctly regardless of the current working directory). Verified with LibreOffice's recalculation engine before each delivery: 0 formula errors.

Eleven sheets:

| sheet | contents |
|---|---|
| Summary | One row per script: headline finding, and whether it reproduces exactly or just the pattern (§12). |
| Swap Isolation | §5's MSE-before/after-by-domain table, plus a bar chart. |
| Addition Flips & Gating Fix | §6.3's per-sample flip list — law-detector score, gated or not, fixed or not — plus recall at the calibrated threshold. |
| MSE by Domain (Addition Impact) | §6.2's fresh aggregate table, plus a bar chart. |
| Multi-Seed Stability | §6.5's full stable-core-vs-shifting-penumbra breakdown across all 9 observed flip points, plus a bar chart with data labels. |
| Capacity Ablation | §6.4's small-vs-large-gate comparison across all three metrics, plus a bar chart. |
| Multi-Dimension Compounding | §7's per-field FPR/recall table and aggregate compounding numbers, plus two bar charts (recall by field, and aggregate rate before/after correction). |
| Real Text Validation | §8's six flip cases with the actual prompt text, domain, and which profiler picked what. |
| Printer Prototype (Calibration) | §8's clean-only-vs-systematic calibration comparison, plus a bar chart. |
| Boundary Solutions Comparison | §6.6's four-mitigation comparison (A/B/C/D), plus a bar chart with data labels — the only one where a bar visibly changes which expert wins top-1 is D. |
| New Profiler Dataset Design | **Design draft, not measured data** — a schema and 20 illustrative rows for what a real calibration dataset should look like, plus sizing guidance computed live via `COUNTIF` formulas (so the guidance panel updates if the draft rows change). Explicitly not a claim about production-scale numbers; see the sheet's own note.

Every percent-change and ratio column is a real Excel formula referencing the adjacent cells (e.g. `=(C5-B5)/B5`), not a pasted-in computed value — checked directly by loading the saved file and confirming cached formula results match what the underlying scripts printed, not just confirming the formulas evaluate without error.
