...
def section_3_addition_and_gating():
    ...
    # Aggregate MSE-by-domain, before vs after addition (replaces reliance on
    # the external versioning_demo.xlsx spreadsheet with a fresh, in-repo
    # equivalent -- same phenomenon, generated by this script, not cited from
    # a file outside this package).
    print("Aggregate MSE by domain, before (frozen v1 routing) vs after (broken jointly-retrained v2 routing):")
    print(f"{'domain':12s} {'MSE before':>12s} {'MSE after':>12s} {'change':>10s}")
    for cn in BASE_NAMES:
        errs_before, errs_after = [], []
        for i in range(len(test[cn]['X'])):
            x = test[cn]['X'][i]; y_true = test[cn]['y'][i]
            p1 = base_clf.predict_proba(base_scaler.transform(x.reshape(1, -1)))[0]
            order1 = [list(base_clf.classes_).index(n) for n in BASE_NAMES]
            idx1, _ = cosine_top1(p1[order1], np.array([e.profile for e in experts_v1]))
            errs_before.append((y_true - experts_v1[idx1].predict(x)[0])**2)

            p2 = joint_clf.predict_proba(full_scaler.transform(x.reshape(1, -1)))[0]
            order2 = [joint_classes.index(n) for n in ALL_NAMES_WITH_LAW]
            idx2, _ = cosine_top1(p2[order2], np.array([e.profile for e in experts_v2]))
            errs_after.append((y_true - experts_v2[idx2].predict(x)[0])**2)
        mb, ma = np.mean(errs_before), np.mean(errs_after)
        print(f"{cn:12s} {mb:12.5f} {ma:12.5f} {100*(ma-mb)/mb:+9.2f}%")
    print()
