...
| `boundary_solutions.py` | §6.6 (adaptive-τ localization) plus **Solution D**, added in this review pass: the gated one-vs-rest fix from §4.3/§6.3, benchmarked head-to-head against the original three mitigations in the same harness. Verified: Solutions A/B/C never change which expert wins top-1 (confirmed again here); Solution D does, correctly fixing 2 of 3 flip cases and leaving the third — the genuinely ambiguous one — largely unresolved, exactly matching §6.3's prediction. This file is a **reconstruction**, not guaranteed byte-identical to the repository's committed version — treat the repo as the source of truth for anything beyond this specific, verified behavior. |

This package's own six scripts plus this document are sufficient to reproduce every load-bearing claim above — no file from the original repository is required. Two places mention that repository's other files (`versioning_demo.xlsx`/`versioning_demo.py` in §6.2, `possibility.md` in §6.4) purely as historical corroboration — additional evidence pointing the same direction, not something any claim in this document depends on. Neither is included here, and neither needs to be.
