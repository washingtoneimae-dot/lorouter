...
`performance_data.xlsx` is generated by `build_workbook.py` from this document's canonical run. It is not a separate data source — every number in it either matches a figure already stated above, or is a fresh aggregate computed from the same run for a view this document doesn't present in table form. Regenerate it any time with `python3 build_workbook.py` (writes next to the script, not to a hardcoded path). Verified with LibreOffice's recalculation engine before each delivery: 0 formula errors.
